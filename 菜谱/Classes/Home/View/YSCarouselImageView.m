...
@property (nonatomic,strong) NSTimer *ddtimer;

@end

@implementation YSCarouselImageView
...
- (void)setUpTimer:(CGFloat )timerInterval {
    ...
    self.ddtimer = [NSTimer timerWithTimeInterval:timerInterval target:self selector:@selector(handleSwipeFromRight) userInfo:nil repeats:YES];
    
    [[NSRunLoop currentRunLoop] addTimer:self.ddtimer forMode:NSDefaultRunLoopMode];
...
- (void)tapImageView {
    ...
    if ([self.delegate respondsToSelector:@selector(dgbCarouselImageView:didSelectItemAtIndex:)]) {
        
        [self.delegate dgbCarouselImageView:self didSelectItemAtIndex:currentPage];
